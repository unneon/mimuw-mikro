...
static_assert(CLICK_LATENCY_US % 1'000 == 0 && CLICK_LATENCY_US >= 0 && CLICK_LATENCY_US <= 255'000, "Click latency must range from 0ms to 255ms with a step of 1ms.");
static_assert(CLICK_WINDOW_US % 1'000 == 0 && CLICK_WINDOW_US >= 0 && CLICK_WINDOW_US <= 255'000, "Click window must range from 0ms to 255ms with a step of 1ms.");
...
static constexpr unsigned char ACCELEROMETER_INITSEQ_3[] = {
    ...
    ((CLICK_THRESHOLD_MG / 500) << 4) | (CLICK_THRESHOLD_MG / 500),
    CLICK_THRESHOLD_MG / 500,
    CLICK_TIMELIMIT_US / 500,
    CLICK_LATENCY_US / 1'000,
    CLICK_WINDOW_US / 1'000,
};
